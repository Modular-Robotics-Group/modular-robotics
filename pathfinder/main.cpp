...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Move Manager initialized." << std::endl
    ...
    try
    {
        ...
        else if (searchMethod == "IDA*" || searchMethod == "IDA_Star" || searchMethod == "IDA_star")
        {
            path = ConfigurationSpace::IDA_Star(&start, &end, heuristic);
        }
        else if (searchMethod == "DFSTT1" || searchMethod == "DFSTT1" || searchMethod == "DFSTT1")
        {
    ...
    std::cout << "Results exported." << std::endl
              << "Cleaning Modules..." << std::endl;
    ModuleIdManager::CleanupModules();
    std::cout << "Modules cleaned." << std::endl
              << "Cleaning Moves..." << std::endl;
