...
if __name__ == "__main__":
    ...
    if args.scenes:
        ...
        parser.error("Provide either --scenes <file> [file2 …] "
                     "or --start_scene <file> [--target_reference <file>].")

    # ------------------------------------------------------------------
    # Single file: just load and pass through – no merging needed
    # ------------------------------------------------------------------
    if len(scene_paths) == 1:
        print(f"Single scene file provided: '{scene_paths[0]}'")
        print("Loading and computing final positions …")
        final_meta, final_modules, final_movements = load_scen_file(scene_paths[0])

    # ------------------------------------------------------------------
    # Two or more files: chain merge
    # ------------------------------------------------------------------
    else:
